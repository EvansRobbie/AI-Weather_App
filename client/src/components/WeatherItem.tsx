...
const WeatherItem: React.FC<weatherProp> = ({
  weather,
  loading,
  prediction,
}) => {
  ...
  const weatherElement = weather.weather.map((currentWeather) => {
    ...
    return (
      <motion.div
        ...
        key={id}
        className="w-full h-full bg-slate-100/50 rounded-xl px-4 py-3 "
      >
        <div className="w-full flex justify-between mb-2 gap-4 ">
          <div className="relative">
            ...
            <h3 className="absolute -right-4 top-4 z-10 opacity-100 text-xs text-cyan-900">
              {weather.sys.country}
            </h3>
          </div>
          <div className="ml-7">
        ...
        <div className="flex w-full justify-around   pb-4 ">
          ...
          <div>
            ...
            <p className="text-4xl text-slate-900 font-semibold">
              {(weather.main.temp - 273.25).toFixed(0)}&#176;
            </p>
            <h5>Temperature</h5>
          </div>
        </div>
